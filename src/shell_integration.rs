...
fn zsh_integration_exists() -> bool {
    let config_dir = dirs::home_dir()
        ...
        .unwrap_or_default();
    let zsh_file = config_dir.join("pm.sh");
    ...
    if let Ok(content) = std::fs::read_to_string(zshrc_path) {
        content.contains("source ~/.config/pm/pm.sh") || content.contains(". ~/.config/pm/pm.sh")
    } else {
        false
    }
}
...
fn create_posix_shell_function_content(is_dev: bool) -> String {
    let (shell_name, function_name, binary_logic, alias_section) = if is_dev {
        ("POSIX shell integration (Development Mode)", "_pm", 
         r#"# Always use _pm binary for development
    local pm_bin="/Users/x/github/zdpk/project-manager/target/debug/_pm"
    
    # Development version uses separate config: ~/.config/_pm/config.yml"#,
         r#"

# PM run alias - Quick extension execution (development)
_pmr() {
    _pm run "$@"
}"#)
    } else {
        ("POSIX shell integration", "pm", 
         r#"# Always use system pm binary
    local pm_bin="pm""#,
         r#"

# PM run alias - Quick extension execution (production)
pmr() {
    pm run "$@"
}"#)
    };
...
async fn setup_zsh_integration() -> Result<()> {
    ...
    let zsh_file_path = config_dir.join("pm.sh");
    let zshrc_path = dirs::home_dir()
        .ok_or_else(|| anyhow!("Could not find home directory"))?
        .join(".zshrc");
    ...
    // Handle existing pm.sh file conflict
    if zsh_file_path.exists() {
        ...
        if !existing_content.contains("# Generated by PM v") {
            ...
            let should_overwrite = Confirm::new("Existing pm.sh file found. Overwrite?")
                .with_default(false)
                .prompt()?;
    ...
    let source_line = "source ~/.config/pm/pm.sh";
    let mut needs_source_line = true;
    
    if zshrc_path.exists() {
        let zshrc_content = fs::read_to_string(&zshrc_path).await?;
        if zshrc_content.contains(source_line) || zshrc_content.contains(". ~/.config/pm/pm.sh") {
...
async fn setup_dev_zsh_integration() -> Result<()> {
    ...
    let zsh_dev_file_path = config_dir.join("pm.sh");
    let zshrc_path = dirs::home_dir()
        .ok_or_else(|| anyhow!("Could not find home directory"))?
        .join(".zshrc");
