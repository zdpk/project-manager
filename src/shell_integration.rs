...
#   _PM_BINARY: Path to PM binary (default: system pm from PATH)
# 
...
function pm
    # Use environment variable if set, otherwise fallback to system pm
    if set -q _PM_BINARY
...
# Ensure function takes precedence over any existing pm command
unset pm 2>/dev/null

pm() {{
    # Use environment variable if set, otherwise fallback to system pm
    local pm_bin="${{_PM_BINARY:-$(command -v pm)}}"
...
/// Create development Fish function content for _pm
fn create_dev_fish_function_content() -> String {
    format!(
        r#"# Generated by PM v{} - Fish shell integration (Development Mode)
# This function enables directory switching with '_pm sw'
#
# Development version of PM for testing and development
# Uses separate config directory: ~/.config/pm-dev/
function _pm
    # Always use _pm binary for development
...
/// Create development Zsh function content for _pm
fn create_dev_zsh_function_content() -> String {
    format!(
        r#"# Generated by PM v{} - Zsh shell integration (Development Mode)
# This function enables directory switching with '_pm sw'
#
# Development version of PM for testing and development
# Uses separate config directory: ~/.config/pm-dev/
unset _pm 2>/dev/null
_pm() {{
    # Always use _pm binary for development
    ...
    if [[ "$1" == "sw" || "$1" == "switch" ]]; then
        ...
        if [[ $pm_status -eq 0 ]]; then
            local new_dir=$(echo "$pm_output" | grep "Switched to:" | sed 's/.*Switched to: //')
            if [[ -n "$new_dir" && -d "$new_dir" ]]; then
                cd "$new_dir"
                echo "📁 Changed directory to: $new_dir"
            else
...
/// Create development Bash function content for _pm
fn create_dev_bash_function_content() -> String {
    format!(
        r#"# Generated by PM v{} - Bash shell integration (Development Mode)
# This function enables directory switching with '_pm sw'
#
# Development version of PM for testing and development
# Uses separate config directory: ~/.config/pm-dev/
...
async fn setup_dev_fish_integration() -> Result<()> {
    ...
    let pm_dev_fish_content = create_dev_fish_function_content();
...
async fn setup_dev_zsh_integration() -> Result<()> {
    ...
    let zsh_dev_content = create_dev_zsh_function_content();
...
async fn setup_dev_bash_integration() -> Result<()> {
    ...
    let bash_dev_content = create_dev_bash_function_content();
