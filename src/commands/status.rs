use crate::config::{load_config, Config, get_machine_id};
use crate::utils::{get_git_current_branch, get_git_remote_url, get_last_git_commit_time, is_git_repository, detect_project_language};
use anyhow::Result;
use serde_json::json;
use std::env;
        ...
        match format {
            ...
            _ => {
                ...
                if quiet {
                    ...
                    let git_str = if let Some(branch) = git_info.get("branch").and_then(|v| v.as_str()) {
                        if git_info.get("has_changes").and_then(|v| v.as_bool()).unwrap_or(false) {
                            format!(" [{}*]", branch)
                        } else {
                            format!(" [{}]", branch)
                        }
                    ...
                    if let Some(branch) = git_info.get("branch").and_then(|v| v.as_str()) {
                        let changes_str = if git_info.get("has_changes").and_then(|v| v.as_bool()).unwrap_or(false) {
                            " (with changes)"
                        } else {
                            ""
                        };
                    ...
                    if let Some(metadata) = machine_metadata {
                        ...
                        if let Some(last_accessed) = metadata.last_accessed.get(&project.id) {
                            println!("🕒 Last accessed: {}", last_accessed.format("%Y-%m-%d %H:%M:%S"));
                        }
                    }
                }
            }
...
fn find_project_by_current_path<'a>(config: &'a Config, current_dir: &PathBuf) -> Option<&'a crate::Project> {
